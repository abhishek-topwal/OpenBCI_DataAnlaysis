...
def EEGnet_MindAmpltd(nb_classes, Chans = 64, Samples = 128, dropoutRate = 0.5, kernLength = 64, F1 = 8,
             D = 2, F2 =16, norm_rate = 0.25, dropoutType = 'Dropout'):
    if dropoutType == 'SpatialDropout2D':
        dropoutType = SpatialDropout2D
    elif dropoutType == 'Dropout':
...
############################### BLOCK1 ############################################
    ...
    block1 = BatchNormalization()(block1)
    block1 = Activation('relu')(block1)
    block1 = MaxPooling2D(pool_size=(1, 3))(block1)
    block1 = dropoutType(dropoutRate)(block1)

############################### BLOCK2 ################################################
    block2 = SeparableConv2D(F2, (1,10),use_bias = False, padding = 'same')(block1)
    block2 = BatchNormalization()(block2)
    block2 = Activation('relu')(block2)
...
############################ Dense ####################################################
    ...
    if nb_classes>1:
        ...
        softmax = Activation('softmax', name='sigmoid')(dense)
    return Model(inputs=input1, outputs=softmax)
